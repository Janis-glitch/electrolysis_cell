...
- Adds an automatic OER fallback via H2O when surface OH− falls below ~1.5×C_MIN_SOLVE_OH_SURF (keeps net OH− balance unchanged).
- Tracks cumulative O₂ produced (n_O2_total) without altering concentration ODEs.
"""
...
C_MIN_ODE_OH = 1e-7   # mol/L – Clamp für OH- im ODE-Integrator
...
class PhysicsCorrectElectrolysisModel:
    ...
    def solve_currents_MT(self, E_anode, c1, c2, c3, c4, i_total):
        ...
        i0_3_eff_base = self.i0_3 * (c4 / c_ref)  # OER base scaling (will be overridden if H2O fallback kicks in)
        ...
        for iteration in range(max_iter):
            ...
            # --- OER fallback trigger: if surface OH− is (nearly) depleted, allow OER via H2O ---
            oer_fallback = (c4_surf <= C_MIN_SOLVE_OH_SURF * 5.0)
            ...
            # Select i0 for OER: in fallback, drop the OH− dependence (use base i0 only)
            i0_3_eff = self.i0_3 if oer_fallback else i0_3_eff_base
            ...
            # R3 (OER): default OH− transport limit
            j_lim3_oh = j_lim_reaction(n3F, {'OH': 4})

            # Optional fallback via H2O when c4_surf is too low: 2 H2O → O2 + 4e− + 4H+
            # Use bulk c_H2O and D_H2O; transport is across geometric area like others.
            if oer_fallback:
                D_H2O = D_values[4]
                c_H2O_m3 = c_H2O0 * 1000.0
                # For stoichiometry ν_H2O = 2
                j_lim3_h2o = (n3F * D_H2O * c_H2O_m3 * inv_delta / 2.0) * area_ratio_geo_to_eff
                # Effective transport ceiling for OER is the less restrictive of both channels
                j_lim3 = max(j_lim3_oh, j_lim3_h2o)
            else:
                j_lim3 = j_lim3_oh
    ...
    def simulate(self, dataset, t_eval=None):
        ...
        # Initial conditions (append trailing 0 for n_O2_total)
        x0 = [dataset.c0['MeOH'], dataset.c0['HCOO'],
              dataset.c0['CO32'], dataset.c0['OH'], dataset.c0['H2O'],
              dataset.c0['HCOO']*V, dataset.c0['CO32']*V, 0, 0]
        ...
        def odes(t, x):
            c1, c2, c3, c4, c5, n_HCOO_total, n_CO32_total, Q_total, n_O2_total = x
            ...
            dQ_dt = i_total
            dn_O2_dt = r3 * self.A_eff

            return [dc1_dt, dc2_dt, dc3_dt, dc4_dt, dc5_dt,
                    dn_HCOO_dt, dn_CO32_dt, dQ_dt, dn_O2_dt]
